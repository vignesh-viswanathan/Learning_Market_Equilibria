...
def createSchedule():
	...
	sched = [[X[int(i * G/NUM_TIMESLOTS + j)] for j in range(int(G/NUM_TIMESLOTS))] for i in range(NUM_TIMESLOTS)]
...
def computeDirectAllocation(Samples, Val, b, c):
	...
	for i in range(N):
		...
		for iter1 in range(len(SamplesCopy)):
			if(np.dot(SamplesCopy[iter1, :], Alloc) != 0):		
				SamplesCopy[iter1, :] = np.array([0]*G)
				ValCopy[iter1, :] = np.array([0]*N)
	...
	for iter1 in range(len(SamplesCopy)):
		if(np.dot(SamplesCopy[iter1, :], Alloc) == 0):
			imax = np.argmax(ValCopy[iter1, :])
			for g in range(G):
				if(SamplesCopy[iter1, g] == 1):
					Alloc[g] = 1
					allocation[imax, g] = 1
...
def computeEnvy(allocation, prices, sched, v, b):
	val_alloc = np.array([[getValueOfBundle(allocation[i,:], v, sched, j) for j in range(N)] for i in range(N)], dtype = 'd')
...
def mainfunc(v, b, dc, samplesize_array, iter_sched):
	...
	optWelfare += getOptWelfare(sched, v)
	# direct_val[TOTAL, 0] = optWelfare 
	for iter_val in range(MAX_ITER):
		...
		for ss in range(len(samplesize_array)):
			...
			direct_val[CONSISTENCY, ss] += checkConsistency(direct_allocation, direct_prices, Samples, Val, sched, v, b)/(MAX_ITER)
			direct_val[UTILITY, ss] += computeUtility(direct_allocation, direct_prices, sched, v, b)/(MAX_ITER)
			direct_val[LOSS, ss] += computeLoss(direct_allocation, direct_prices, dc, sched, v, b)/(MAX_ITER)
			direct_val[ENVY, ss] += computeEnvy(direct_allocation, direct_prices, sched, v, b)/(MAX_ITER) 
			direct_val[BURNT, ss] += computeBurnt(direct_allocation, direct_prices)/(MAX_ITER)

	return direct_val, optWelfare
...
for dc in dclist:
	...
	print(dc, end = "\r")
	direct_values = []
	results = Parallel(n_jobs=num_cores)(delayed(mainfunc)(v, b, dc, samplesize_array, iter_sched) for iter_sched in range(MAX_SCHED))
	for iter_sched in range(MAX_SCHED):
		optWelfare.append(results[iter_sched][1])
		direct_values.append(results[iter_sched][0])

	f_out.write(dcName[dclist.index(dc)] + ":\n")
	f_out.write("Opt_avg = " + str(np.mean(optWelfare)) + "\n")
	f_out.write("Opt_var = " + str(np.var(optWelfare, ddof = 1)) + "\n")
