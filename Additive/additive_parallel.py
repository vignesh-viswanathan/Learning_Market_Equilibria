...
def computeIndirectValue(Samples, Val, b, vreal):
	v = np.array([[0]*G for i in range(N)], dtype = 'd')
	for i in range(N):
		reg = LinearRegression(fit_intercept = False).fit(Samples, Val[:, i])
		v[i, :] = np.array(reg.coef_)
	...
	vdash = v[:, Gdash]
	y = cp.Variable((N, len(Gdash)))
	p = cp.Variable(len(Gdash))
	Nones = np.ones(N)
	Gones = np.ones(len(Gdash))
	constraints = []
	...
	obj = cp.Maximize(cp.sum([y[i,g]*cp.log(vdash[i,g]) for i in range(N) for g in range(len(Gdash))]) + cp.sum([cp.entr(p[g]) for g in range(len(Gdash))]))
	prob = cp.Problem(obj, constraints)
	prob.solve(solver = cp.SCS, max_iters = 5000)

	x = np.array([[0]*G for i in range(N)], dtype = 'd')
	for i in range(N):
		for g in range(len(Gdash)):
			if(y.value[i, g] > 0.00001):
				x[i, Gdash[g]] = y.value[i, g]/p.value[g]
...
def mainfunc(v, b, dc, samplesize_array, iter_sched):
	...
	indirect_val = np.array([0]*len(samplesize_array), dtype = 'd')
	...
	for ss in range(len(samplesize_array)):
		...
		indirect_val[ss] += computeIndirectValue(Samples, Val, b, v)
...
for dc in dclist:
	...
	f_out.write("OptEq_avg = " + str(divEqWelfare) + "\n")
	...
	for ss in range(len(samplesize_array)):
		temp = []
		for iter_sched in range(MAX_ITER):
			temp.append(indirect_values[iter_sched][ss])
		temp_avg.append(np.mean(temp))
		temp_var.append(np.var(temp, ddof = 1))
	f_out.write("Utility_avg = " + printProper(temp_avg) + "\n")
	f_out.write("Utility_var = " + printProper(temp_var) + "\n")
